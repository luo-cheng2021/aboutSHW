...
class StubOP : public ov::op::Op {
    std::string m_mode;
    int m_axis;

public:
    OPENVINO_OP("StubOP");
    // OPENVINO_FRAMEWORK_MAP(onnx, "StubOP", {}, {});
    // OPENVINO_FRAMEWORK_MAP(pytorch, "StubOP", {}, {});
    StubOP() = default;
    StubOP(const ov::OutputVector& args) : Op({args}) {
    ...
    std::shared_ptr<ov::Node> clone_with_new_inputs(const ov::OutputVector& new_args) const override {
        return std::make_shared<StubOP>(new_args);
    }
...
OPENVINO_CREATE_EXTENSIONS(
    std::vector<ov::Extension::Ptr>({
        ...
        std::make_shared<ov::OpExtension<StubOP>>(),
        // Register operaton mapping, required when converted from framework model format
        std::make_shared<ov::frontend::OpExtension<StubOP>>()
    }));